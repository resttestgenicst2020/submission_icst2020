...
class deletePet_200 {
	...
	@Test
	public void deletePet_TEST_200() throws Exception {
	{
		// Test Step getPets
	...
	}
	{
		...
		ExecutionResult executionresult1174586025;
		try {
			// API Call
			Object returnValue_executionresult1174586025 = new PetApi().deletePetWithHttpInfo(long407148497);
			executionresult1174586025 = apiResponseParser.parseApiResponseObject(ReflectionHelper.getMethodByName(PetApi.class, "deletePetWithHttpInfo"), returnValue_executionresult1174586025);
		} catch (InvocationTargetException e) {
			// Here, if request executed with http error code
			Throwable targetException = e.getTargetException();
			if (targetException.toString().contains("io.swagger.client.ApiException")){
				executionresult1174586025 = apiResponseParser.parseApiExceptionObject(targetException);
			} else {
				throw new OperationExecutionException("Exception during the execution of operation", e);
			}
		} catch (IllegalAccessException e) {
		...
		assertTrue(executionresult1174586025.getStatusCode() == 200);
	
		assertEquals(0, executionresult1174586025.getValidationErrors().size());
